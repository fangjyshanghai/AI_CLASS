 ...
 
{
    ...
    ofstream outfile("D:\\图像像素.txt",ios::in|ios::trunc);
     ...
     { 
      ...
      for(L1=hang;L1>hang-8;L1--)//8*8矩阵行
      {
        ...
        m=*(pBmpBuf+L1*lineByte+L2);
                             outfile<<setw(4)<<setiosflags(ios::left)<<m;
                 count_xiang_su++;
...
const int M = 64; 
const int N = 64;//定义一个边为M的数组 
int a[M][N] = {0}; //将数组初始化 
int temp[M][N] = {0};//将数组初始化 
void PrintArray(int a[M][N], int len1, int len2) //打印数组 
{
...
void HandleArray(int a[M][N], int m, int n) //处理数组 
{
    for(int i = 2; i < m; ++i)
    {
       for(int j = 2; j < n; ++j)
       {
           a[i][j] += temp[i - 2][j] + temp[i + 2][j] + temp[i][j - 2] + temp[i][j + 2];
       }
    }
}
...
int main()
{
    ...
    //PrintArray(a, M, N); //打印数组 
    clock_t start = clock();
    HandleArray(a, M, N); //按照要求处理数组 
    cout << "The Handled array is:" << endl;
   // PrintArray(a, M, N);//打印数组 
    clock_t end = clock();
     
    cout << "it costs: " << (double)(end - start) / CLOCKS_PER_SEC << "S" << endl;
